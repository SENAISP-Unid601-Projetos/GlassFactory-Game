...
Maquina.prototype = {
    ...
    ligarMaquina: function() {
        ...
        // Atualizar disponibilidade dos itens antes de configurar seleção
        this.atualizarDisponibilidadeItens();
        
        // Configurar primeiro a seleção de itens para garantir que o status seja inicializado corretamente
    ...
    configurarSelecaoItens: function() {
        ...
        // Atualizar inicialmente a disponibilidade de todos os itens
        this.atualizarDisponibilidadeItens();
        ...
        conteudoProcesso.addEventListener('click', function(event) {
            ...
            if (item) {
                // Verificar se o item está desabilitado (quantidade = 0)
                if (item.classList.contains('desabilitado')) {
                    mostrarAviso("Este item não está disponível no inventário!", 2000);
                    return;
                }
    ...
    atualizarDisponibilidadeItens: function() {
        const tipoCapitalizado = this.tipo.charAt(0).toUpperCase() + this.tipo.slice(1);
        const conteudoProcesso = document.getElementById(`conteudoProcesso${tipoCapitalizado}`);
        
        if (!conteudoProcesso) return;
        
        // Encontrar qual data attribute usar
        let dataAttr = null;
        if (conteudoProcesso.querySelector('[data-receita]')) {
            dataAttr = 'data-receita';
        } else if (conteudoProcesso.querySelector('[data-mistura]')) {
            dataAttr = 'data-mistura';
        } else if (conteudoProcesso.querySelector('[data-vidro]')) {
            dataAttr = 'data-vidro';
        } else if (conteudoProcesso.querySelector('[data-produto]')) {
            dataAttr = 'data-produto';
        }
        
        if (!dataAttr) return;
        
        // Para cada item, verificar disponibilidade no inventário
        const itens = conteudoProcesso.querySelectorAll(`[${dataAttr}]`);
        itens.forEach(item => {
            const itemId = item.getAttribute(dataAttr);
            let quantidade = 0;
            
            // Para o misturador, todos os itens estão sempre disponíveis
            if (this.tipo === 'misturador') {
                quantidade = 1; // Sempre disponível
            } else {
                quantidade = SistemaInventario.obterQuantidade(itemId) || 0;
            }
            ...
            if (quantidade === 0) {
                ...
                const infoElement = item.querySelector('.item-info');
                if (infoElement) {
                    const originalHtml = infoElement.getAttribute('data-original') || infoElement.innerHTML;
                    infoElement.setAttribute('data-original', originalHtml);
                    infoElement.innerHTML = '<span style="color:#e74c3c">Indisponível</span>';
                }
            } else {
                ...
                // Restaurar informação original se existir
                const infoElement = item.querySelector('.item-info');
                if (infoElement && infoElement.hasAttribute('data-original')) {
                    infoElement.innerHTML = infoElement.getAttribute('data-original');
                }
            }
        });
    },
